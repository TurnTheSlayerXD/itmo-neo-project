...
sol_hash=$(neo-go contract deploy -i ./solution_token/main.nef --out ./solution_token/deploy.json -m ./solution_token/neo.manifest.json -w ../wallets/wallet1.json -r http://localhost:30333 | grep 'Contract' | cut -d ' ' -f 2)
